...
### 表格 formatter 渲染与 XSS 防护回归（2026-07-28）

| 编号 | 问题与影响 | 修复 | 回归结果 |
|---|---|---|---|
| FE-TABLE-01 | `MesTable` 使用未注册的 `$base.escapeHtml()` 渲染 formatter 列。存在 formatter 的页面会在渲染期报错，表现为接口和导出均有数据、分页总数正确，但表格行全部空白；`View_DefectItemDistribute` 的 12 条数据因此不可见。 | 在 `main.js` 同时注册历史兼容别名 `$base`；`MesTable` 统一通过容错方法执行 formatter，单列 formatter 异常不会再中断整张表。 | 缺陷分布接口返回的 12 行数据已确认可正常格式化；新增日期、数字及异常 formatter 用例通过。 |
| FE-TABLE-02 | formatter 原先直接使用 `v-html`，若返回接口或用户输入，可能执行不可信 HTML，存在 XSS 风险；此前直接转义又会使系统内固定的操作链接、状态图标显示为 HTML 字符串。 | 普通 formatter 改为纯文本渲染；仅源码中固定的 5 处操作/状态富文本显式标记 `trustedHtml`；工单关联单号等业务数据拼接进富文本前强制转义。 | 固定操作链接与状态样式保留，业务文本默认不执行 HTML。 |

本次执行完整 Vue 单元测试，结果为 **316 通过、0 失败、0 跳过**；生产构建成功。构建输出中的包体积、旧版深度选择器及 Browserslist 提示为既有警告，不影响本次回归结论。

### 看板外部跳转页回归（2026-07-28）

| 编号 | 问题与影响 | 修复 | 回归结果 |
|---|---|---|---|
| FE-BOARD-01 | “车间生产管控”和“工单执行进度”页面进入后立即执行 `window.open()` 跳转至已不可达的外部 Anji Report 服务；页面内备用链接误用 `ref` 属性，无法手动打开。 | 移除自动跳转，改为在 iMES 页面明确说明其依赖独立大屏服务；备用链接改为带 `target="_blank"`、`rel="noopener noreferrer"` 的正确 `href`。 | 新增两个看板跳转页单测通过；完整 Vue 单元测试为 **318 通过、0 失败、0 跳过**。 |

说明：外部大屏服务的源码和部署文件不在当前项目中，服务恢复后只需更新对应链接地址；本次修复确保服务未部署时不会再自动跳至错误页。

### 生产单据打印模板回归（2026-07-29）

| 编号 | 问题与影响 | 根因 | 修复 | 复测结果 |
|---|---|---|---|---|
| FE-PRINT-01 | `Production_SalesOrder`（销售订单）和 `Production_AssembleWorkOrder`（装配工单）点击打印后不显示预览，打印库提示“请传递 id 参数且配置 getContentUrl 并确保接口有数据返回”；`Production_ProductPlan`（生产计划）正常。 | 三个页面均已正确传递各自主键 `id`，且共用 `Print-Designer/print.html`。打印模块再按 `cat` 查找 `Base_PrintTemplate` 中 `StatusFlag=1` 的模板：销售订单的“销售订单打印”模板存在但状态为 `0`；装配工单仅有打印分类、没有模板；生产计划有启用模板。因此后端取得空模板内容并解析为 JSON，触发空值异常，前端展示了泛化提示。 | 新增 `20260729-fix-production-print-templates.sql`：启用销售订单内置模板，并根据已验证的销售订单表格布局创建装配工单默认模板，绑定 `AssembleWorkOrderCode` 和产品明细字段。新建库脚本 `iMES-current.docker.sql` 已包含该修复；后端打印模板接口同时增加空模板保护，未来配置缺失时返回明确中文提示，不再抛空值异常。 | 当前本地数据库已执行迁移并重建 API 容器。销售订单（ID 13）、装配工单（ID 8）、生产计划（ID 6）的“模板、单据头、明细”接口均返回成功；不存在的分类返回“未找到分类……的可用打印模板”提示。API Docker 镜像编译通过。 |
| FE-PROD-QTY-01 | 新建/编辑生产工单时“计划数”为 `0`，或装配工单明细“数量”为 `0`，原页面和 API 均会放行。装配工单随后读取进度时，数据库函数以 `ReportQty / PlanQty` 计算百分比，零计划数量会触发 SQL Server 除以零异常，页面表现为保存后加载失败。 | Vue 提交前钩子未校验数量；两个生产服务也未校验正数；实体的 `[Required]` 只能禁止空值，不能禁止 `0`；原进度函数未处理历史零计划数量。 | 最小范围增加三层保护：两个 Vue 页面在新增、编辑前拒绝非正整数；工单和装配工单服务在新增、编辑时拒绝 `<= 0`；新增 `20260730-fix-production-zero-quantity.sql`，使历史零/负计划数量读取进度时返回 `0%`，并已合入 `iMES-current.docker.sql`。不修改正常数量、工单状态或自动生成逻辑。 | 新增 `PROD-WO-06`、`PROD-AWO-03` 覆盖新增/编辑零数量阻断；Vue 全量单测 **320/320 通过**。API Docker 镜像编译成功并已重建容器。数据库事务临时插入计划数为 `0` 的工单，进度函数返回两行 `0%`，随后回滚，未保留数据。 |

人工界面验收：在网页中分别选中一条销售订单、装配工单、生产计划后点击“打印”，新窗口应展示单据编号和产品明细；无需重启 Vue 前端。打印模板属于数据库配置，若未来在“打印模板管理”中禁用或删除默认模板，接口会提示需要配置并启用模板。
