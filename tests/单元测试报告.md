...
## iMES.Production 单元测试复测结果
| 项目 | 结果 |
| --- | --- |
| 执行日期 | 2026-07-22 |
| 基线 commit SHA | `b608912610133fcb71e9675475ec3dd57b3650df` |
| 测试工程 | `源码/iMES.Net/iMES.Production.Tests` |
...
| 编号 | 问题 |  当前处理 | 复测 |
| --- | --- | --- | --- |
| UT-PROD-FIX-01 | `Production_ProductPlanRepositoryTests` 的 `TestDbContext` 继承 `BaseDbContext`，但 `Production_ProductPlanRepository` 构造参数为 `SysDbContext`，类型不匹配无法编译 | 将 `TestDbContext` 改为继承 `SysDbContext`，通过传入 `DbContextOptions<BaseDbContext>` 绕过 `OnConfiguring` 连接真实数据库 |

|改动文件 | 改动内容 |
| `源码/iMES.Net/iMES.Production.Tests/Production_ProductPlanRepositoryTests.cs` | ① `TestDbContext` → `TestSysDbContext : SysDbContext`；② 构造函数接收 `DbContextOptions<BaseDbContext>` 传给 `base(options)`；③ 重写 `OnConfiguring` 当 options 已配置时跳过父类逻辑；④ 字段类型保持 `SysDbContext` |
...
| --- | --- |
| 复测日期 | 2026-07-22 |
| 复测分支 | `modify-retest/Production-regression`（基于 `dev` 最新） |
| 复测命令 | `dotnet test 源码/iMES.Net/iMES.Production.Tests/iMES.Production.Tests.csproj --no-restore` |
| 复测结果 | **31 通过，0 失败** |
| 生产代码是否修改 | 否（`Production_ProductPlanRepository.cs` 保持原 `SysDbContext` 依赖不变） 

**结论：** 生产领域 31 条单元测试在当前 `dev` 基线全部通过，测试夹具调整后无需修改生产代码，复测通过。
