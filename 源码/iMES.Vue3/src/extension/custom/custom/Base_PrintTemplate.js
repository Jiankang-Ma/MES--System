      ...
      render() {
        return [
          h(resolveComponent('el-alert'), {
            ...
            closable: false, title: '先编辑打印模版，之后切换默认的模版，然后在具体业务表单进行打印（目前仅在销售订单集成了模版套打，其他待开发）'
          }, ''),
        ]
      }
    },
    ...
    onInit() {  //框架初始化配置前，
      ...
      this.buttons.splice(3, 0, {  //也可以用push或者splice方法来修改buttons数组
        ...
        onClick: function () {
          ...
          window.open(this.http.ipAddress + 'Print-Designer/index.html?id=' + selectRow[0].PrintTemplateId + '&cat=' + this.result.catalogCode, '_blank')
        }
      });
