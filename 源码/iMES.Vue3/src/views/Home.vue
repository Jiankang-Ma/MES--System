<template>
    ...
    <div>
      ...
      <div class="order-range">
        ...
        >
          ...
          <div class="date">计划数:{{ item.PlanQty }}</div>
          <div class="date">良品数:{{ item.GoodQty }}</div>
          <div class="date">不良品数:{{ item.NoGoodQty }}</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="home-contianer">
    <div class="h-chart">
      ...
      <div class="h-top-center" style="width: 100%">
        <div class="n-item">
          <div
...
export default {
  ...
  created() {
    ...
    this.getVersionInfo()
    this.http
      .get('api/Base_Process/getAppHomeProcessTop5', {}, true)
      .then((result) => {
    ...
    this.http
      .get('api/Base_DefectItem/getAppHomeDefectValue', {}, true)
      .then((item) => {
        let dataDefect = item.map((key) => ({
          name: key.name,
          value: key.data,
        }))
