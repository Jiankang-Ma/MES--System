<template>
	<view class="order-detail">
		...
		<view class="order-detail-list">
			<u-button class="loginBtn" style="margin-top:10rpx;" icon="checkmark" type="primary" ripple-bg-color="#909399"  @click="save">保存</u-button>
	...
	export default {
		data() {
			return {
				...
				AssembleWorkOrder_Id: null, //编辑时传过来的订单id
				//主表配置
				//主表配置
				editFormFields: {
					"AssembleWorkOrderCode": "",
					"Remark": ""
				},
				editFormOptions: [{
						"title": "装配工单号",
						"required": true,
						"field": "AssembleWorkOrderCode"
					},
					{
						"title": "备注",
						"required": true,
				...
				rows: [], //数据
				columns: [{
						field: 'AssembleWorkOrderList_Id',
						title: '明细表主键',
						hidden: true
					},
					{
		...
		methods: {
			save() { //保存
				let url = ''
				if (this.AssembleWorkOrder_Id) { //编辑操作
					url = "api/Production_AssembleWorkOrder/update"
				} else {
					url = "api/Production_AssembleWorkOrder/add"
				}
				let params = {
			...
			confirmDel() { //删除行数据
				let url = "api/Production_AssembleWorkOrder/delDetail?AssembleWorkOrderList_Id=" + this.currentRow.AssembleWorkOrderList_Id;
			...
			getOrderData() { //获取主表数据(现在用的是框架的方法，可以自己写接口返回数据)
				...
				params.wheres = JSON.stringify([{
					name: "AssembleWorkOrder_Id",
					value: this.AssembleWorkOrder_Id
				}]);
				this.http.post("api/Production_AssembleWorkOrder/getPageData", params, true).then(result => {
			...
			getOrderListData() { //获取明细表数据(现在用的是框架的方法，可以自己写接口返回数据)
				...
				//生成查询参数
				params.value = this.AssembleWorkOrder_Id;
				this.http.post("api/Production_AssembleWorkOrder/getDetailPage", params, true).then(result => {
		...
		onShow() {
			...
			if (params && params.AssembleWorkOrder_Id) {
				this.AssembleWorkOrder_Id = params.AssembleWorkOrder_Id;
				//获取主表数据
				this.getOrderData();
				//获取明细表数据
				this.getOrderListData();
			} else {
				this.AssembleWorkOrder_Id = null;
			}
			uni.setNavigationBarTitle({
				title: this.AssembleWorkOrder_Id ? '装配工单编辑' : '新建装配工单'
			})
		}
	}
</script>
