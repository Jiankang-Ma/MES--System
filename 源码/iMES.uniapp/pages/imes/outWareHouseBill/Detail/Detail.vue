	...
	export default {
		data() {
			return {
				...
				OutWareHouseBill_Id: null, //编辑时传过来的订单id
				//主表配置
				//主表配置
				editFormFields: {
		...
		methods: {
			save() { //保存
				let url = ''
				if (this.OutWareHouseBill_Id) { //编辑操作
					url = "api/Ware_OutWareHouseBill/update"
				} else {
					url = "api/Ware_OutWareHouseBill/add"
				}
				let params = {
			...
			confirmDel() { //删除行数据
				let url = "api/Ware_OutWareHouseBill/delDetail?OutWareHouseBillList_Id=" + this.currentRow.OutWareHouseBillList_Id;
			...
			getOrderData() { //获取主表数据(现在用的是框架的方法，可以自己写接口返回数据)
				...
				params.wheres = JSON.stringify([{
					name: "OutWareHouseBill_Id",
					value: this.OutWareHouseBill_Id
				}]);
				this.http.post("api/Ware_OutWareHouseBill/getPageData", params, true).then(result => {
			...
			getOrderListData() { //获取明细表数据(现在用的是框架的方法，可以自己写接口返回数据)
				...
				//生成查询参数
				params.value = this.OutWareHouseBill_Id;
				this.http.post("api/Ware_OutWareHouseBill/getDetailPage", params, true).then(result => {
		...
		onShow() {
			...
			if (params && params.OutWareHouseBill_Id) {
				this.OutWareHouseBill_Id = params.OutWareHouseBill_Id;
				//获取主表数据
				this.getOrderData();
				//获取明细表数据
				this.getOrderListData();
			} else {
				this.OutWareHouseBill_Id = null;
			}
			uni.setNavigationBarTitle({
				title: this.OutWareHouseBill_Id ? '出库单编辑' : '新建出库单'
			})
		}
	}
</script>
