	...
	export default {
		data() {
			return {
				...
				ProductPlan_Id: null, //编辑时传过来的订单id
				//主表配置
				//主表配置
				editFormFields: {
					"ProductPlanCode": "",
		...
		methods: {
			save() { //保存
				let url = ''
				if (this.ProductPlan_Id) { //编辑操作
					url = "api/Production_ProductPlan/update"
				} else {
					url = "api/Production_ProductPlan/add"
				}
				let params = {
			...
			confirmDel() { //删除行数据
				let url = "api/Production_ProductPlan/delDetail?SalesOrderList_Id=" + this.currentRow.SalesOrderList_Id;
			...
			getOrderData() { //获取主表数据(现在用的是框架的方法，可以自己写接口返回数据)
				...
				params.wheres = JSON.stringify([{
					name: "ProductPlan_Id",
					value: this.ProductPlan_Id
				}]);
				this.http.post("api/Production_ProductPlan/getPageData", params, true).then(result => {
			...
			getOrderListData() { //获取明细表数据(现在用的是框架的方法，可以自己写接口返回数据)
				...
				//生成查询参数
				params.value = this.ProductPlan_Id;
				this.http.post("api/Production_ProductPlan/getDetailPage", params, true).then(result => {
		...
		onShow() {
			...
			if (params && params.ProductPlan_Id) {
				this.ProductPlan_Id = params.ProductPlan_Id;
				//获取主表数据
				this.getOrderData();
				//获取明细表数据
				this.getOrderListData();
			} else {
				this.ProductPlan_Id = null;
			}
			uni.setNavigationBarTitle({
				title: this.ProductPlan_Id ? '计划编辑' : '新建计划'
			})
		}
	}
</script>
